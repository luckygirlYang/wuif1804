...
//明星单品方法二：
{
    var danpin = document.querySelector('.danping');
    let left=document.querySelector(".danping-top-right-zuo");
    let right=document.querySelector(".danping-top-right-you");
    let lisbox=document.querySelector('.danping-list-box');
    let listItem=document.querySelectorAll('.danping-list');
    let lw=(parseInt(getComputedStyle(listItem[0],null).width) + parseInt(getComputedStyle(listItem[0],null).marginRight)) * 5
    let time = 0;
    let max =listItem.length / 5;
    left.onclick = function () {
        if(--time < 0){
            time = 0;
        }
        lisbox.style.transform  = `translateX(${-time * lw}px)`;
    }
    right.onclick = function () {
        console.log(time);
        if(++time == max) {
...
//为你推荐方法二：
{
    let zuo=document.querySelector(".anjiankuang-zuo");
    let you=document.querySelector(".anjiankuang-you");
    let lists=document.querySelector(".center-right2");
    let listItems=document.querySelectorAll('.center-right2-list');
    let lew=(parseInt(getComputedStyle(listItems[0],null).width) + parseInt(getComputedStyle(listItems[0]).marginRight))*5;
    console.log(lew);
    let times = 0;
    let Max = listItems.length / 5;
    zuo.onclick = function () {
        if(--times < 0){
            times = 0;
        }
        lists.style.transform=`translateX(${-times * lew}px)`;
    }
    // console.log(zuo);
...
//内容方法二：
{
    let tulist = document.querySelector('.tushu');
    console.log(tulist);
    let width = parseInt(getComputedStyle(tulist,null).width);
    let lis = document.querySelectorAll('.tusu .shum');
    let btn = document.querySelectorAll('.neiron-dian .neiron-dian2');
    let leftt = document.querySelector('.neiron-jiantou');
    let rightt = document.querySelector('.neiron-jiantou1');
    let now=0;
    let next=0;
    ...
    function move(){
        ...
        btn[now].classList.remove('neiron-dian1');
        btn[next].classList.add('neiron-dian1');
        now=next;
    }
